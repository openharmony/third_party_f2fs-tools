				  ...
				  struct f2fs_node *node)
{
	...
	if (err) {
		...
		DBG(0, "inode[0x%x] remove corrupted xattr entry succes!\n", ino);
	}
...
static bool chk_and_update_verity_blk_reached(struct child_info *child)
{
...
static bool chk_verity_node_reached(struct child_info *child, int idx, struct f2fs_inode *inode)
{
		...
		struct node_info *ni, struct child_info *child_d)
{
	...
	if (compressed && (ftype == F2FS_FT_DIR || ftype == F2FS_FT_REG_FILE)) {
		...
		if (i_log_cluster_size != F2FS_MIN_COMPRESS_LOG_SIZE) {
			ASSERT_MSG("[0x%x] wrong i_log_cluster_size=0x%x",
						nid, i_log_cluster_size);
			if (c.fix_on) {
				node_blk->i.i_log_cluster_size = F2FS_MIN_COMPRESS_LOG_SIZE;
				need_fix = 1;
				FIX_MSG("ino[0x%x] set i_log_cluster_size %d", nid, F2FS_MIN_COMPRESS_LOG_SIZE);
			}
		}
	...
	for (idx = 0; idx < 5; idx++) {
		...
		if (dedup_supported && is_need_check_dedup_nid(&child) &&
			!chk_verity_node_reached(&child, idx, &node_blk->i)) {
			if (i_nid != 0 && c.fix_on) {
				/*
				 * the node id before the one contain verity data will be reserved,
...
check:
	...
	if (child.state & FSCK_UNMATCHED_EXTENT) {
		...
		ASSERT_MSG("ino: 0x%x has wrong ext: [pgofs:%u, blk:%u, len:%u], verity data start offset: %u",
		...
		struct f2fs_compr_blk_cnt *cbc, struct child_info *child)
{
	...
	for (i = 0; i < NIDS_PER_BLOCK; i++) {
		...
		if (is_need_check_dedup_nid(child) && !chk_verity_node_reached(child, DIRECT_NODE_IDX, inode)) {
			if (i_nid != 0 && c.fix_on) {
				/*
				 * no modifacations will be made be temporarily:
				 *  node_blk->in.nid[i] = 0;
				 *  need_fix = 1;
				 */
		...
		struct f2fs_compr_blk_cnt *cbc, struct child_info *child)
{
	...
	for (i = 0; i < NIDS_PER_BLOCK; i++) {
		...
		if (is_need_check_dedup_nid(child) && !chk_verity_node_reached(child, INDIRECT_NODE_IDX, inode)) {
			if (i_nid != 0 && c.fix_on) {
				/*
				 * no modifacations will be made be temporarily:
				 *  node_blk->in.nid[i] = 0;
...
int fsck_verify(struct f2fs_sb_info *sbi)
{
	...
	if (force || (c.fix_on && f2fs_dev_is_writable())) {
		...
		ClearExtraFlag(sbi->raw_super, EXTRA_NEED_FSCK_FLAG);
		ret = 0;
	}
	return ret;
}
